...
| QMS-010 | **Controlled traceability matrix** (Hazard → RC → Requirement → Architecture → Unit → Verification) maintained as scattered tables, not one tool-backed, version-controlled matrix | ⚠️ | Consolidate into a single maintained matrix so an auditor can trace any item end-to-end and detect gaps; ideally generated/checked in CI. |
